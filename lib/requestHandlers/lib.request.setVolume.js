...
module.exports = class Request_SetVolume extends RequestMediaRenderer
{
    ...
    async runAction(_resolve, _reject, _mediaRendererVirtual, _mediaRendererRoom, _roomUdn)
    {
        ...
        var equalRooms = parseInt(this.getQueryValue("equalRooms", 0));
        ...
        if(_mediaRendererVirtual && equalRooms)
        {
            ...
            for(var rendererIdx=0; rendererIdx<rendererUdns.length; rendererIdx++)
            {
                try
                {
                    ...
                    {
                        var result = await mediaRendererRoom.setVolume(value * self.volumeMultiplier(), self.waitTillConfirmed());
                        resultSum += result;
                    }
                }
                catch(_exception)
        ...
        else if (_mediaRendererVirtual)
        {
            if(this.useRelativeValue())
            {
                _mediaRendererVirtual.getVolume().then(function(_volumeNow){
                    _mediaRendererVirtual.setVolume(parseInt(_volumeNow) + (parseInt(value) * self.volumeMultiplier()), self.waitTillConfirmed()).then(function(_data){
                            _resolve(_data);
                        }).catch(function(_data){
                            _reject(_data);
                        });
            ...
            {
                _mediaRendererVirtual.setVolume(value *  self.volumeMultiplier(), self.waitTillConfirmed()).then(function(_data){
                        _resolve(_data);
                    }).catch(function(_data){
                        _reject(_data);
                    });
        ...
        else
        {
            if(this.useRelativeValue())
            {
                _mediaRendererRoom.getVolume().then(function(_volumeNow){
                    _mediaRendererRoom.setVolume(parseInt(_volumeNow) + (parseInt(value) * self.volumeMultiplier()), self.waitTillConfirmed()).then(function(_data){
                            _resolve(_data);
                        }).catch(function(_data){
                            _reject(_data);
                        });
            ...
            {
                _mediaRendererRoom.setVolume(value *  self.volumeMultiplier(), self.waitTillConfirmed()).then(function(_data){
                        _resolve(_data);
                    }).catch(function(_data){
                        _reject(_data);
                    });
