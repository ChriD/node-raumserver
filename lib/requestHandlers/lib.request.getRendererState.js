...
module.exports = class Request_GetRendererState extends RequestLongPolling
{
    ...
    runAction(_resolve, _reject)
    {
        try
        {
            ...
            var onlyRaumfeld    = this.getQueryValue("onlyRaumfeld"); 
            ...
            else
            {
                ...
                if(getRealRenderers)
                {
                    for (var udn of this.managerDisposer.deviceManager.mediaRenderers.keys())
                    {
                        ...
                        {
                            // skip 3rd party renderers
                            if(onlyRaumfeld && !mediaRenderer.isRaumfeldRenderer())
                                continue;
                            dataArray.push(this.createReturnData(mediaRenderer, mediaRenderer.rendererState))
                        }
                    }
    ...
    createReturnData(_mediaRenderer, _rendererState)
    {
        ...
        // we may get the info from a zone rendere wher the attached rooms are stored in the 'rooms' array
        if(_rendererState.rooms)
        {
            for(var udn in _rendererState.rooms)
            {
                var roomInfoFromZone    = this.createRoomInfoFromUdn(udn);
        ...
        // or we may have a 'normal' renderer which could have a relation to a room
        else
        {
            var roomInfoFromZone  = this.createRoomInfoFromUdn(_mediaRenderer.udn());
            newStateObject.rooms.push(roomInfoFromZone);
        }
    ...
    createRoomInfoFromUdn(_udn)
    {
        var roomInfoFromZone = {};
        var roomObject  = this.managerDisposer.zoneManager.getRoomObjectFromMediaRendererUdnOrName(_udn);
