...
module.exports = class Request_EnterManualStandby extends RequestMediaRenderer
{
    constructor()
    {
        super();
    ...
    {       
        ...
        {
            _mediaRendererRoom.enterManualStandby().then(function(_data){
                    _resolve(_data);
        ...
        else
        {
            ...
            for(var rendererIdx=0; rendererIdx<rendererUdns.length; rendererIdx++)
            {
                ...
                if(mediaRendererRoom)
                {
                    try
                    {                                        
                        var result = await mediaRendererRoom.enterManualStandby();
                        resultSum += result;
                    }
                    catch(_exception)
                    {
